...
def migrate_legacy_db():
    legacy_db = "licenses.db"
    if os.path.exists(legacy_db) and not os.path.exists(DB_FILE):
        try:
            shutil.copy(legacy_db, DB_FILE)
            os.rename(legacy_db, legacy_db + ".bak")
            print(f"[DATABASE] Migrated existing database from root to '{DB_FILE}'")
        except Exception as e:
            print(f"[DATABASE] Error migrating legacy database: {e}")
...
def init_db():
    with get_db() as conn:
        ...
        if "user_note" not in columns:
            conn.execute("ALTER TABLE licenses ADD COLUMN user_note TEXT")
        if "last_activated_at" not in columns:
...
def dashboard():
    ...
    with get_db() as conn:
        ...
        cursor.execute("SELECT COUNT(*) FROM licenses WHERE is_active = 1 AND datetime(expires_at) >= datetime(?)", (now_str,))
        active = cursor.fetchone()[0]
        
        cursor.execute("SELECT COUNT(*) FROM licenses WHERE datetime(expires_at) < datetime(?)", (now_str,))
        ...
        cursor.execute("""
            ...
            WHERE action LIKE 'Verification Success%' AND date(created_at) = ?
        """, (today_date,))
        ...
        for i in range(5, -1, -1):
            ...
            cursor.execute("""
                SELECT COUNT(*) FROM activity_logs 
                WHERE action LIKE 'Verification Success%' AND strftime('%Y-%m', created_at) = ?
            """, (month_str,))
...
def licenses_view():
    ...
    if status_filter == "active":
        query += " AND is_active = 1 AND datetime(expires_at) >= datetime(?)"
        params.append(now_str)
    elif status_filter == "blocked":
        query += " AND is_active = 0"
    elif status_filter == "expired":
        query += " AND datetime(expires_at) < datetime(?)"
        params.append(now_str)
    ...
    with get_db() as conn:
        ...
        cursor.execute('''
            SELECT 
                ...
                COUNT(*) as total,
                SUM(CASE WHEN is_active = 1 AND datetime(expires_at) >= datetime(?) THEN 1 ELSE 0 END) as active,
...
def settings_view():
    ...
    return render_template("settings.html", active_page="settings", backups=backups)
...
def settings_update_appearance():
    ...
    with get_db() as conn:
        if title:
            conn.execute("INSERT OR REPLACE INTO settings (key, value) VALUES ('dashboard_title', ?)", (title,))
        if logo:
            conn.execute("INSERT OR REPLACE INTO settings (key, value) VALUES ('dashboard_logo', ?)", (logo,))
        conn.commit()
...
def settings_regenerate_api():
    ...
    with get_db() as conn:
        conn.execute("INSERT OR REPLACE INTO settings (key, value) VALUES ('api_key', ?)", (new_key,))
        conn.commit()
...
def settings_create_backup():
    ...
    try:
        ...
        log_activity(session["username"], f"Database: Created database backup snapshot '{filename}'")
        flash(f"Database snapshot '{filename}' successfully captured!", "success")
    except Exception as e:
        flash(f"Database snapshot backup failed: {e}", "danger")
...
def settings_restore_backup(filename):
    ...
    if not os.path.exists(backup_path):
        flash("Restore failed: Backup file does not exist.", "danger")
        return redirect(url_for("settings_view"))
...
def settings_delete_backup(filename):
    ...
    if os.path.exists(backup_path):
        try:
            ...
            log_activity(session["username"], f"Database: Deleted backup file '{filename}'")
            flash("Backup file permanently removed.", "success")
        except Exception as e:
            flash(f"Error removing backup file: {e}", "danger")
    else:
